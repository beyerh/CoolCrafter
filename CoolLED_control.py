...
class WorkingThread(QtCore.QThread):
    ...
    def light(self):
        ...
        for i, task in enumerate(self.seqlist):
            ...
            self.update_signal.emit("Working: " + str(i + 1) + '/' + str(len(self.seqlist)) + '   ')
            if self.q.empty() == False:
                print('Quit on user request!')

                break
            elif task[0] == 'W':
                self.msleep(int(float(task[1:]) * self.frametime * 1000 + mean(lossarray) * 1000))
...
class GUI (QtWidgets.QWidget):
    ...
    def setGridLayout (self):
        ...
        instructions = QtWidgets.QLabel("LED 1: A \t LED 2: B \t LED 3: C \t LED 4: D \txI###: set intensity for channel x \t W###: wait\nEnter your sequence in the following format: \n\nW20 A2 C2 A2 C2 A2 C2 A5 W100 etc. (freely)")
        
        self.useframes = QtWidgets.QCheckBox('Use frames instead of seconds')
        self.useframes.setChecked(False)
        self.useframes.stateChanged.connect(self.setframemode)
        
        frameboxl = QtWidgets.QLabel("frame time (s): ")
        self.framebox = QtWidgets.QLineEdit()
        ...
        self.seqbox = QtWidgets.QPlainTextEdit()
        ...
        self.sbaredit = QtWidgets.QLabel("Idle.       ")
              
        self.sbar = QtWidgets.QStatusBar()
    ...
    def doplot(self):
        ...
        for count, item in enumerate(self.seqlistnoint):
            if item[0] == 'W':
                ...
                regions.append(pg.LinearRegionItem(values=[totalcount, totalcount + float(item[1:])*self.frametime], orientation='vertical', brush=brushc, movable=False, bounds=None))
                totalcount = totalcount + float(item[1:])*self.frametime
    ...
    def runLEDs(self):
        ...
        self.workingthread.update_signal.connect(self.updatesbar)
    ...
    def openfile(self):
        ...
        if self.path:
            f = QtWidgets.QFileDialog.getOpenFileName(self, "Open File", self.path, "pE Control files (*.pE2)")[0]
        else:
        ...
        if f:
            self.path = os.path.dirname(f) # store path for next time
